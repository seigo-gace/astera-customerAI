...
class InternalAudit:
    def check(self, packet: SharedRolePacket, results) -> list[str]:
        violations=[]; fact_ids={f.fact_id for f in packet.facts}
        for result in results:
            if any(eid not in fact_ids for eid in result.evidence_ids): violations.append("unsupported_claim")
            for resolution in result.task_resolutions:
                if any(eid not in fact_ids for eid in resolution.evidence_ids): violations.append("unsupported_claim")
        return list(dict.fromkeys(violations))


class CustomerAIInternalCore:
    def __init__(self, *, decomposer: TaskDecomposer, search: SearchPlanner, grounding: GroundingPlanner, skills: SkillRegistry, roles, integrator: DialogueIntegrator, gate: CompletionGate, state: StateStore, japanese, max_targeted_retry: int = 1):
        self.decomposer=decomposer; self.search=search; self.grounding=grounding; self.skills=skills; self.roles=roles; self.integrator=integrator; self.gate=gate; self.state=state; self.japanese=japanese; self.max_targeted_retry=max(0,max_targeted_retry)
        self.audit=InternalAudit(); self.composer=FinalAnswerComposer(); self.satisfaction=RuntimeSatisfactionGate(); self.security=PublicBoundary(); self.refiner=WritingRefiner()
    ...
    async def run(self,session_id:str,message:str)->FinalResponse:
        request_id="req_"+uuid.uuid4().hex; turn_id="turn_"+uuid.uuid4().hex; state=self.state.get(session_id)
        prepared=self.japanese.prepare(message,state.as_japanese_context()); normalized_text=str(prepared["normalized_text"])
        ...
        capsules=self.skills.select(SkillQuery(language=language,audience=audience,tasks=tuple(contract.need_tasks),has_evidence=bool(facts),text_length=len(normalized_text)))
        packet=SharedRolePacket(request_id=request_id,session_id=session_id,turn_id=turn_id,user_message=message,normalized_need=contract.target,audience=audience,tasks=contract.need_tasks,user_conditions=dict(state.user_conditions),language_hints={"term_candidates":prepared["term_candidates"],"ellipsis":prepared["ellipsis"],"search_terms":grounding_plan.search_terms,"skill_terms":skill_plan.search_terms},facts=facts,completion_conditions=contract.completion_conditions)
        try:
            results=await self.roles.run_all(packet,capsules); integrated=self.integrator.integrate(results)
        except Exception:
            return self._failure(request_id,session_id,turn_id,ResolutionMode.RUNTIME_FAILURE,"runtime_failure",["role_runtime_failure"])
        external=self.audit.check(packet,results); quality=self.gate.evaluate(packet,integrated,external_violations=external); retries=0
        while not quality.passed and retries<self.max_targeted_retry:
            ...
            try:
                ...
                integrated=self.integrator.integrate(results); external=self.audit.check(repair_packet,results); quality=self.gate.evaluate(repair_packet,integrated,external_violations=external)
            except Exception: break
            retries+=1
        missing_inputs=tuple(item for task in contract.need_tasks for item in task.required_user_inputs if item)
        plan=IntegratedAnswerPlan(needs=tuple(contract.need_tasks),resolutions=integrated.resolutions,blocked_task_ids=tuple(integrated.contradiction_task_ids),missing_evidence_task_ids=quality.missing_evidence_task_ids,missing_user_inputs=missing_inputs if quality.resolution_score<1.0 else ())
        composed=self.composer.compose(plan); answer=self.refiner.refine(composed.answer or "") if composed.answer else ""; terminology=self.japanese.terminology_violations(answer) if answer else []; security=self.security.check_output(answer=answer,forbidden_literals=packet.forbidden_claims,unexecuted_completion_claim=False)
        major=[t for t in contract.need_tasks if t.priority=="primary"]; resolved=set(composed.resolved_task_ids); all_major=all(t.task_id in resolved for t in major); actionable=all((not t.actionability_required) or any(r.task_id==t.task_id and bool(r.action_steps) for r in integrated.resolutions) for t in contract.need_tasks); evidence_complete=not quality.missing_evidence_task_ids; fp_ok="false_premise_uncorrected" not in quality.violations
        sat_ok,sat_violations=self.satisfaction.evaluate(composed.mode,RuntimeSatisfactionSignals(all_major,evidence_complete,True,actionable,fp_ok,unsupported_claim_count=int("unsupported_claim" in quality.violations),terminology_violation_count=len(terminology)))
        violations=list(dict.fromkeys([*quality.violations,*sat_violations,*security.violations,*("terminology_violation" for _ in terminology)])); passed=bool(quality.passed and sat_ok and security.passed and not terminology)
        if passed: self.state.update(session_id,active_topics=[t.intent for t in contract.need_tasks],last_user_need=contract.target,user_conditions=dict(state.user_conditions))
        failure_class=None
        if not passed:
