...
class HFChatClient:
    """Role-scoped OpenAI-compatible chat client.

    Constructive/Adversarial keep the existing HF Router path.
    Evidence-bound can use the local llama.cpp 8B endpoint.
    """
    ...
    def __init__(
        ...
        *,
        ...
        local = api_url.startswith(("http://127.0.0.1", "http://localhost"))
        if not token.strip() and not local and client is None:
