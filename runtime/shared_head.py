...
_ROLE_RULES = {
    RoleName.CONSTRUCTIVE: "Resolve every task. Return public_text, evidence_ids, conditions, exceptions and required action_steps. Never invent Astera facts.",
    RoleName.ADVERSARIAL: "Inspect the constructive draft. Find missing needs, contradictions, false premises, failure conditions and unclear wording. Do not rewrite the whole answer.",
    RoleName.EVIDENCE_BOUND: "Inspect the constructive draft claim-by-claim against supplied facts. Flag unsupported, stale, conditional or conflicting claims. Preserve evidence conditions and exceptions.",
}
_VALIDATOR_CAPS = {
...
class SharedHeadRolePool:
    """One 8B head, two inference waves: Constructive -> two parallel validators."""

    def __init__(self, client: HFChatClient):
        self.client = client
    ...
    async def _run(self, role: RoleName, packet: SharedRolePacket, skills: Sequence[CapabilityCapsule], *, draft: RoleResult | None = None) -> RoleResult:
        prompt = {
            "role": role.value,
            ...
            "output": "RoleResult JSON: role, claims, evidence_ids, risks, uncertainties, missing_needs, contradictions, task_resolutions, completion_state",
        }
        result = await self.client.chat_json([
            {"role": "system", "content": "You are one isolated role inside Astera Customer AI. Return JSON only. Do not expose chain-of-thought. Do not claim external actions were executed."},
    ...
    async def run_all(self, packet: SharedRolePacket, skills: Sequence[CapabilityCapsule]) -> list[RoleResult]:
    ...
    async def validate_draft(self, packet: SharedRolePacket, skills: Sequence[CapabilityCapsule], draft: RoleResult) -> list[RoleResult]:
        adversarial, evidence = await asyncio.gather(
    ...
    async def retry_role(self, role: RoleName, packet: SharedRolePacket, skills: Sequence[CapabilityCapsule]) -> RoleResult:
    ...
    async def semantic_decompose(self, message: str, seed: TaskContract) -> TaskContract:
        prompt = {
            ...
            "required_fields": ["purpose", "target", "conditions", "constraints", "premises", "missing_information", "success_criteria", "need_tasks", "completion_conditions"],
            "rule": "Refine only the task structure. Keep every user need and constraint. Do not invent product facts. need_tasks must use t1,t2... and valid response_shape values.",
        }
        obj = await self.client.chat_json([
