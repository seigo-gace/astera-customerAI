...
async def lifespan(_: FastAPI):
    ...
    if service.settings.notion_token:
        try:
            version = f"hp-public-{int(time.time())}"
...
app = FastAPI(
    title="Astera Customer AI Public Boundary",
    version="3.0.0",
    lifespan=lifespan,
    docs_url=None,
    redoc_url=None,
...
async def readyz() -> dict:
    ...
    ready = bool(
        ...
        and checks.get("v8")
        and checks.get("kb")
        and (not checks.get("model_enabled") or hf_api_available)
    )
    if not ready:
        raise HTTPException(
            status_code=503,
            detail={"ready": False, "checks": checks, "hf_api_available": hf_api_available},
        )
    return {
        "ready": True,
        "checks": {
