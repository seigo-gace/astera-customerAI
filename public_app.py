...
PUBLIC_RATE_LIMIT = max(
    1,
    int(os.getenv("CUSTOMER_AI_PUBLIC_SESSION_REQUESTS_PER_MINUTE", "30")),
)
...
def _allow_request(session_id: str) -> bool:
    ...
    if len(_RATE_BUCKETS) > 1024:
        stale = [
            ...
            if not values or now - values[-1] >= _RATE_WINDOW_SECONDS
        ]
...
async def lifespan(_: FastAPI):
    ...
    if service.settings.notion_token:
        try:
            ...
            LOGGER.info(
                "CUSTOMER_AI_PUBLIC_KB_SYNC_OK version=%s source_pages=%s",
                synced.get("version"),
                synced.get("source_pages"),
            )
        except Exception:
...
async def readyz() -> dict:
    ...
    return {
        ...
        "checks": {
            ...
            "v8": True,
            "kb": True,
            "model_enabled": checks.get("model_enabled", False),
        },
    }
...
async def public_respond(payload: MessagePayload) -> dict:
    ...
    request_id = "public_" + hashlib.sha256(
        f"{request.session_id}:{request.message_id}".encode()
    ).hexdigest()[:32]
    ...
    return {
        ...
        "public_source": result.get("public_source")
        or result.get("public_sources"),
