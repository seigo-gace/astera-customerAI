...
def test_clean_first_launch_passes() -> None:
    _guard().validate()


def test_failed_retry_without_fix_evidence_is_blocked() -> None:
    with pytest.raises(ValueError, match="repeat_failure_without_fix_evidence_blocked"):
        _guard(
            previous_job_id="job-123",
            previous_failure_fingerprint=SHA_C,
        ).validate()


def test_failed_retry_with_fix_evidence_passes() -> None:
    _guard(
        previous_job_id="job-123",
        previous_failure_fingerprint=SHA_C,
...
def test_same_failure_state_without_fix_is_blocked() -> None:
    with pytest.raises(ValueError, match="same_failure_retry_blocked"):
        require_new_failure_state(
            previous_failure_fingerprint=SHA_C,
            proposed_failure_fingerprint=SHA_C,
            fix_evidence_sha256="",
        )


def test_same_failure_state_with_fix_is_allowed() -> None:
    require_new_failure_state(
        previous_failure_fingerprint=SHA_C,
        proposed_failure_fingerprint=SHA_C,
        fix_evidence_sha256=SHA_A,
...
def test_failure_fingerprint_is_whitespace_and_case_stable() -> None:
...
def test_incomplete_launch_contract_fails_closed(field, value, message) -> None:
