...
@pytest.mark.asyncio
async def test_public_response_is_synchronous_and_returns_only_public_contract(
    monkeypatch,
):
...
async def test_public_response_rejects_non_hp_source():
    with pytest.raises(HTTPException) as error:
        await public_app.public_respond(
            payload(source="astera-api")
        )
    assert error.value.status_code == 422


@pytest.mark.asyncio
async def test_public_session_delete_clears_runtime_context(
    monkeypatch,
):
    deleted = []
    monkeypatch.setattr(
    ...
    result = await public_app.delete_public_session(
        "session_public_123456"
    )
